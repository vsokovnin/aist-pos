...
# Гейт эха: вариант ответа не пересобирает вопрос дословно — иначе человек выбирает по совпадению слов.
python3 "$repo/scripts/audit/echo-check.py" || {
  echo "СБОРКА ОСТАНОВЛЕНА: вариант ответа дословно повторяет вопрос"; exit 1; }

# Гейт первого разговора: шаги задачи, вопросы и матрица согласованы между собой.
python3 "$repo/scripts/audit/check-first-talk.py" "$repo" || {
  echo "СБОРКА ОСТАНОВЛЕНА: первый разговор разошёлся с каталогом"; exit 1; }
...
# Гейт трассировки: каждая ячейка матрицы подтверждена канонической трассой.
